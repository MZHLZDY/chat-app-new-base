...
// Data Board (mengganti todo langsung)
...
const loadBoards = async () => {
    ...
    try {
        const response = await ApiService.get("chat/boards");
        const data = response.data?.data ?? response.data;
        boardList.value = Array.isArray(data) ? data.slice(0, 4) : [];
    } catch (error) {
...
<template>
    <div
        ...
        :class="{ 'dark-mode': currentThemeMode === 'dark', loaded: isLoaded }"
    >
        <div class="dashboard-grid">
            <!-- Welcome Card with Gradient Background -->
            <div
                class="welcome-card card-base animate-slide-up"
            ...
            >
                ...
                <div class="welcome-content">
                    ...
                    <h4 class="welcome-subtitle">
                        Kelola semua percakapan Anda dalam satu tempat
                    </h4>
                </div>
                <div class="button-wrapper">
                    <router-link
                        :to="{ name: 'dashboard.private-chat' }"
                        class="btn-chat btn-chat-private"
                    >
                        <MessageCircle class="btn-icon" />
                        <span>Chat Pribadi</span>
                    </router-link>
                    <router-link
                        :to="{ name: 'dashboard.group-chat' }"
                        class="btn-chat btn-chat-group"
                    >
            ...
            <!-- Board Card -->
            <div
                class="todo-card card-base animate-slide-left"
            ...
            >
                <div class="todo-header">
                    <div class="todo-header-left">
                        ...
                        <div>
                            <h3 class="todo-title">Todo Board</h3>
                            <p class="todo-subtitle">
                                {{ boardList.length }} board aktif
                            </p>
                        </div>
                    </div>
                    <router-link
                ...
                <div class="todo-body">
                    <!-- Loading -->
                    <div v-if="isLoadingBoard" class="loading-state">
                    ...
                    >
                        <router-link
                            ...
                            :key="board.id"
                            to="/dashboard/todo-list"
                            class="board-item animate-fade-in"
                            :style="{
                                animationDelay: `${index * 0.05}s`,
                                borderLeftColor: board.color || '#667eea',
                            }"
                        >
                            <div class="board-emoji">
                                {{ board.icon || "📋" }}
                            </div>
                    ...
                    <div v-else class="empty-state">
                        ...
                        <h4 class="empty-title">Belum Ada Board</h4>
                        <p class="empty-text">
                            Buat board pertama kamu di halaman Todo List
                        </p>
                    </div>
                </div>
            </div>
            ...
            <!-- Phone Card -->
            <div
                class="info-card phone-card card-base animate-slide-up"
            ...
            <div
                class="info-card time-card card-base animate-slide-up"
            ...
            <div
                class="info-card stats-card card-base animate-slide-up"
...
.dark-mode .todo-icon-wrapper {
    background: linear-gradient(
        135deg,
        rgba(16, 164, 250, 0.1) 0%,
        rgba(16, 164, 250, 0.05) 100%
    );
}
...
.dark-mode .stat-green {
    background: linear-gradient(
        135deg,
        rgba(80, 205, 137, 0.15) 0%,
        rgba(80, 205, 137, 0.08) 100%
    );
}
...
.dark-mode .stat-blue {
    background: linear-gradient(
        135deg,
        rgba(0, 158, 247, 0.15) 0%,
        rgba(0, 158, 247, 0.08) 100%
    );
}
